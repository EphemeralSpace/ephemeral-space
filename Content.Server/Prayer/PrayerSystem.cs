...
public sealed partial class PrayerSystem : EntitySystem
{
    ...
    public void SendSubtleMessage(ICommonSession target, ICommonSession? source, string messageString, string popupMessage)
    {
        ...
        _chatManager.SendChatMessage(messageString, target, ESSharedChatSystem.LocalChannel);
    ...
    public void Pray(ICommonSession sender, PrayableComponent comp, string message)
    {
        ...
        _chatManager.SendAdminMessage($"{Loc.GetString(comp.NotificationPrefix)} <{sender.Name}>: {message}");
