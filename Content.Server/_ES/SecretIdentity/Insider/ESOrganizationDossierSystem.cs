...
public sealed partial class ESOrganizationDossierSystem : EntitySystem
{
    ...
    private static readonly ProtoId<ESOrganizationPrototype> CrewOrganization = "Crew";

    /// <inheritdoc/>
    public override void Initialize()
    {
        SubscribeLocalEvent<ESOrganizationDossierComponent, MapInitEvent>(OnMapInit);
    }

    private void OnMapInit(Entity<ESOrganizationDossierComponent> ent, ref MapInitEvent args)
    {
        ...
        var crewMinds = _secretIdentity.GetOrganizationMembers(CrewOrganization).ToList();
        ...
        var notCrewMinds = _secretIdentity.GetNotOrganizationMembers(CrewOrganization).ToList();
    ...
    private EntityUid SpawnBriefing(Entity<ESOrganizationDossierComponent> ent)
    {
        var paper = Spawn(ent.Comp.PaperPrototype);
        var text = Loc.GetString("es-organization-dossier-briefing-text",
        ...
        _metaData.SetEntityName(paper, Loc.GetString("es-organization-dossier-briefing-name"));
        return paper;
    }

    private EntityUid SpawnClueFile(Entity<ESOrganizationDossierComponent> ent, Entity<ESCharacterComponent?> mind, string codeName)
    {
        var paper = Spawn(ent.Comp.PaperPrototype);
        _paper.SetContent(paper, GetClueMessage(ent, mind, codeName).ToMarkup());
        _metaData.SetEntityName(paper, Loc.GetString("es-organization-dossier-name", ("name", codeName)));
        return paper;
    }

    private FormattedMessage GetClueMessage(Entity<ESOrganizationDossierComponent> ent, Entity<ESCharacterComponent?> mind, string codeName)
    {
