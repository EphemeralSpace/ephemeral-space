...
public sealed partial class ESTargetOrganizationObjectiveSystem : EntitySystem
{
    ...
    {
        SubscribeLocalEvent<ESTargetOrganizationObjectiveComponent, ESValidateObjectiveTargetCandidates>(OnValidateTarget);
    }

    private void OnValidateTarget(Entity<ESTargetOrganizationObjectiveComponent> ent, ref ESValidateObjectiveTargetCandidates args)
    {
        ...
        if ((_secretIdentity.GetOrganizationOrNull(args.Candidate) != ent.Comp.Organization) ^ ent.Comp.Invert)
            args.Invalidate();
    }
}
